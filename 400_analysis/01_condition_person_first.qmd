...
- The total number of uses of person-first language we observe is somwhat higher in tabloids and lower in broadsheets than we would expect based on the word count in these subcorpora, but this result is not strongly significant (p < 0.05).
...
```{r}
fp_test(
  wc1 = diff_boot$condition_first,
  wc2 = diff_boot$person_first,
  label1 = "condition",
  label2 = "person", 
  dist = mydistribution
)
...
```{r}
fp_test(
  wc1 = diff_boot$freq_cond_first,
  wc2 = diff_boot$freq_pers_first,
  label1 = "condition",
  label2 = "person", 
  dist = mydistribution
)
