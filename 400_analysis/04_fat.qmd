...
So, yes, the frequency of use of the word "fat" is lower in broadsheets than in tabloids. Are articles that use the word "fat" longer in broadsheets than tabloids?
...
```{r}
chisq_instances_wc_normalised(fat_annotated_filtered, metadata_full, source_type)
```
...
```{r}
chisq_articles_totalart_normalised(fat_annotated_filtered, metadata_full, source_type)
```
...
```{r}
chisq_instances_wc_normalised(fat_annotated_filtered, metadata_full, orientation)
```
...
```{r}
chisq_articles_totalart_normalised(fat_annotated_filtered, metadata_full, orientation)
```
